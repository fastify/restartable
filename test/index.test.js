...
test('should create and restart fastify app', { concurrency: 1 }, async (t) => {
  ...
  t.assert.deepStrictEqual(host, `http://127.0.0.1:${COMMON_PORT}`)
  t.assert.deepStrictEqual(app.addresses()[0].address, '127.0.0.1')
  t.assert.deepStrictEqual(app.addresses()[0].port, COMMON_PORT)
...
test('should create and restart fastify app twice', async (t) => {
  ...
  async function createApplication (fastify, opts) {
    ...
    app.addHook('onClose', async () => {
      if (++closeCounter > 1) {
        t.assert.fail('onClose hook called more than once')
      }
      t.assert.deepStrictEqual(app.closingRestartable, closingRestartable)
...
test('should create and restart fastify https app', async (t) => {
  ...
  const host = await app.listen({ host: '127.0.0.1', port: COMMON_PORT })
  t.assert.deepStrictEqual(host, `https://127.0.0.1:${COMMON_PORT}`)
  t.assert.deepStrictEqual(app.addresses()[0].address, '127.0.0.1')
  t.assert.deepStrictEqual(app.addresses()[0].port, COMMON_PORT)
...
test('logger', async (t) => {
...

  {
    ...
    t.assert.deepStrictEqual(level, 30)
    t.assert.deepStrictEqual(msg, `Server listening at ${host}`)
  }
})
...
test('should contain a persistentRef property', async (t) => {
  ...
  async function createApplication (fastify, opts) {
    ...
    if (app.restarted) {
      t.assert.deepStrictEqual(app.persistentRef, proxy)
    } else {
  ...
  t.assert.deepStrictEqual(firstPersistentRef, proxy)
  ...
  t.assert.deepStrictEqual(proxy.persistentRef, proxy)
